...
async function parseData(input) {
    ...
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });
      ...
      const prompt = "Given a string extract the releavent information (Image, Surname, Name,Passport Number, D.O.B,Gender, Nationality and Expiration Date)from it and convert to a json object.";
...
export const scanVisa = async (req, res) =>{
  ...
  try {
    ...
    //   const str = await scanGPTData(text);
      ...
      const userData = {
        ...
        country:data.nationality ? data.nationality : null,
      ...
      const newUserData = new Visa(userData);
